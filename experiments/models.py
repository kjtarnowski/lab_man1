...
class Experiment(models.Model):
    PROGRESS_CHOICES = (("TBD", "TBD"), ("ONGOING", "ONGOING"), ("UC", "UC"), ("TO_REP", "TO_REP"))
    comments = models.TextField(default="-")
    experiment_date = models.DateField(default=None, blank=True, null=True)
    created = models.DateTimeField(auto_now_add=True)
    updated = models.DateTimeField(auto_now=True)
    compound = models.ForeignKey(Compound, on_delete=models.CASCADE, related_name="experiments_for_compound")
    lab_person = models.ForeignKey(LabPerson, on_delete=models.CASCADE, related_name="experiments_of_person")
    aparat = models.ForeignKey(Aparat, on_delete=models.CASCADE, related_name="experiments_on_this_aparat")
    ...
    progress = models.CharField(max_length=10, choices=PROGRESS_CHOICES, default="TBD")
    ...
    class Meta:
        constraints = [models.UniqueConstraint(fields=["compound", "experimental_set"], name="compoud_from_set")]

    def get_absolute_url(self):
        return reverse("experiments:editExperiment")

    def __str__(self):
        return "_".join([self.experimental_set.name, self.compound.name, str(self.id)])
